<script lang="ts">
	...
	let { data, onClose, onRefresh = invalidateAll, returnHref }: {
		data: ThreadViewData;
	...
	async function markUnread() {
		if ((await patch({ isRead: false }))?.ok) await leave(backHref);
	}
	...
	async function toggleArchive() {
		...
		try {
			...
			void leave(data.archived ? '/inbox?view=archive' : '/inbox');
		} catch {
	...
	async function trash() {
		if ((await patch({ trashed: true }))?.ok) await leave(backHref);
	}

	async function restore() {
		if ((await patch({ trashed: false }))?.ok) await leave('/inbox');
	}
	...
	async function reportSpam() {
		...
		try {
			...
			void leave(inboxCategoryPath(latest?.category ?? 'primary'));
		} catch {
			error = t('mailbox.conversationNetwork');
		}
	}
	...
	async function moveTo(category: (typeof INBOX_CATEGORIES)[number]) {
		...
		try {
			...
			void leave(inboxCategoryPath(category));
		} catch {
			error = t('mailbox.conversationNetwork');
		}
	}
	...
	function goBack() {
		if (onClose) { void onClose(); return; }
...
<div class="mail-page">
	<header class="mail-toolbar">
		...
		<div class="toolbar-actions">
			{#if latest && !data.trashed && !data.spam}<SnoozeControl ids={[latest.id]} snoozed={Boolean(data.snoozedUntil)} onDone={() => { void leave('/inbox'); }} />{/if}
			<button
				type="button"
				class="icon-btn"
				class:starred
